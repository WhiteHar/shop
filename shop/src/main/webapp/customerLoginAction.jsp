...
<%
	...
	// 정보 삽입(set)
	member.setId(id);	// 21번째 줄에서 선언한 id값을 Customer 형식의 member변수 내에 저장
	member.setPass(pass);
	...
	// CustomerDao 사용
	Customer loginCustomer = customerDao.login(member);	// loginCustomer: member변수로 customerDao 내의 login 메서드를 실행
	
	// DB에 일치하는 id가 없을 때
	if(loginCustomer == null){
		System.out.println("로그인 실패");
		response.sendRedirect(request.getContextPath()+"/loginForm.jsp?errorMsg=please login agin");
		return;
	}
